...
const CartMain = ({ data }) => {
  ...
  const [total, setTotal] = useState({
    price: cart.reduce(
      (prev, curr) =>
        Math.ceil((Number(prev) + Number(curr.priceTotal)) * 100) / 100,
      0
    ),
    ...
    setCart((cart) => {
      return cart.map((product) => {
        if (product.id === id) {
          ...
          let newTotalPrice = Math.ceil(
            ((product.count + 1) * product.price * 100) / 100
          );
      ...
      return cart.map((product) => {
        if (product.id === id) {
          addedDataToArr(
            id,
            value,
            Math.ceil(value * product.price * 100) / 100
          );
          return {
            ...product,
            count: value,
